...
class AddressBook:
	def __init__(self, win):
		...
		self.searchInp = Entry(self.AddressbookFrame, textvariable=self.searchVal ,width=30).place(x=20, y=50)

		#searchIcon = PhotoImage(file="images/search.png")
		#searchIconImg = searchIcon.subsample(1, 1)
		self.searchBtn = Button(self.AddressbookFrame, text="Search", command=self.search).place(x=205, y=48)

		refreshImg = Image.open("images/refresh.png")
		refreshRender = ImageTk.PhotoImage(refreshImg)
		self.refreshBtn= Button(self.AddressbookFrame, image=refreshRender, command=self.fetchContact)
		self.refreshBtn.image=refreshRender
		self.refreshBtn.place(x=255,y=48)

		#self.refreshBtn= Button(self.AddressbookFrame, text= "Refresh", command=self.fetchContact).place(x=245,y=47)
	...
	def exportContacts(self):
		...
		try:
			...
			csvFile.close()
			messagebox.showinfo("Success","Successfully exported")
		except:
			print("Error: ", sys.exc_info())
